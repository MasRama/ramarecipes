...
<?php if(session('errors')) {
 foreach(session('errors') as $key => $value) {
  ...
  break;
    ...
    <form action="<?= base_url('resep/tambah') ?>" method="POST" enctype="multipart/form-data" class="space-y-6">
        <label class="block">
             <label class="font-medium text-lg" for="tk-form-elements-name">Gambar Masakan</label>
             <input class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-indigo-100 file:text-indigo-700 hover:file:bg-indigo-300" 
             type="file" id="gambar" name="gambar" />
        </label>

        <div class="space-y-1">
            <label class="font-medium text-lg" for="tk-form-elements-name">Nama Masakan</label>
            <input class="w-96 block border border-gray-200 rounded px-3 py-2 leading-6 focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50" 
            type="text" id="nama" placeholder="Masukkan nama masakan" name="nama" />
        </div>

        <div class="space-y-1">
            <label class="font-medium text-lg" for="tk-form-elements-textarea">Bahan-bahan</label>
            <textarea class="w-full block border border-gray-200 rounded px-3 py-2 focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50" 
            id="bahan" rows="4" placeholder="Masukkan Bahan-bahan masakan" name="bahan"></textarea>
        </div>

        <div class="space-y-1">
            <label class="font-medium text-lg" for="tk-form-elements-textarea">Cara memasak</label>
            <textarea class="w-full block border border-gray-200 rounded px-3 py-2 focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50" 
            id="masak" rows="4" placeholder="Jelaskan langkah-langkah memasak nya" name="masak"></textarea>
        </div>

        <div class="space-y-1">
            <label class="font-medium text-lg" for="tk-form-elements-textarea">Deskripsi</label>
            <textarea class="w-full block border border-gray-200 rounded px-3 py-2 focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50" 
            id="desc" rows="4" placeholder="Deskripsi singkat masakan" name="desc"></textarea>
        </div>
